...
```json
"check": "bun run lint && bun run typecheck && bun run format:check && bun run test && bun run build"
```
...
- **build**: Proves the project compiles. Run before pushing.

If `check` script doesn't exist, run whatever lint/typecheck/test/build scripts are in package.json.
